...
interface CompletedSavRequestsProps {
  completedSav: ServiceAfterSale[]
  onSelectSav: (id: string) => void
  getStatusBadge: (status: string) => JSX.Element
}

export function CompletedSavRequests({
  completedSav,
  onSelectSav,
  getStatusBadge
}: CompletedSavRequestsProps) {
  return (
    <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
      <div className="p-6 border-b border-gray-200">
        <h3 className="text-lg font-medium">Demandes terminées</h3>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Résolution</th>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date de résolution</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {completedSav.map((sav) => (
              <tr key={sav.$id} className="hover:bg-gray-50 cursor-pointer" onClick={() => onSelectSav(sav.$id)}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {sav.completionDate ? new Date(sav.completionDate).toLocaleDateString() : '-'}
                </td>
              </tr>
            ))}
            
            {completedSav.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">Aucune demande terminée</td>
              </tr>
            )}
          </tbody>
        </table>
